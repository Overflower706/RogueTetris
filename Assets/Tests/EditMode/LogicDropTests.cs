...
public class LogicDropTests
{
    ...
    {
        // 각 블록 타입별로 Drop 후 실제 위치와 보드 상태를 검증
        ...
        Assert.AreEqual(3, afterTDropData.board.GetBlock(5, 1), "T블록 위치 (5,1)에 블록 존재");

        // 간단히 나머지 블록들도 Drop되는지 확인
        foreach (var blockType in new[] { TetriminoType.S, TetriminoType.Z, TetriminoType.J, TetriminoType.L })
        {
            logicManager.RestartGame();

            gameData = logicManager.GetGameData();
            var testBlock = new Tetrimino(blockType);
            ...
            Assert.AreEqual(0, testBlock.position.y, $"{blockType} 블록이 바닥에 도달해야함");
        }
    }
    ...
    public void DropTetrimino_ShouldWorkWithRotatedTetriminos()
    {
        ...
        // T 블록 회전 후 Drop 테스트
        logicManager.RestartGame(); // 보드 초기화
        gameData = logicManager.GetGameData();
        var tBlock = new Tetrimino(TetriminoType.T);
        tBlock.position = new Vector2Int(5, 15);
    ...
    public void DropTetrimino_MultipleDrop_ShouldStackCorrectly()
    {
        ...
        // 첫 번째 블록: I블록을 바닥에 Drop
        var firstBlock = new Tetrimino(TetriminoType.I);
        ...
        Assert.AreEqual(1, afterFirstDrop.board.GetBlock(4, 0), "첫 번째 I블록 (4,0)에 고정됨");
        Assert.AreEqual(1, afterFirstDrop.board.GetBlock(5, 0), "첫 번째 I블록 (5,0)에 고정됨");
        Assert.AreEqual(1, afterFirstDrop.board.GetBlock(6, 0), "첫 번째 I블록 (6,0)에 고정됨");
        Assert.AreEqual(1, afterFirstDrop.board.GetBlock(7, 0), "첫 번째 I블록 (7,0)에 고정됨");

        // 두 번째 블록: O블록을 같은 영역에 Drop (스택되어야 함)
        var secondBlock = new Tetrimino(TetriminoType.O);
        ...
        Assert.AreEqual(1, afterSecondDrop.board.GetBlock(4, 0), "첫 번째 I블록이 여전히 (4,0)에 있음");
        Assert.AreEqual(1, afterSecondDrop.board.GetBlock(7, 0), "첫 번째 I블록이 여전히 (7,0)에 있음");

        // O블록이 I블록 위에 스택되었는지 확인 (y=1에 위치해야 함)
        Assert.AreEqual(2, afterSecondDrop.board.GetBlock(5, 1), "O블록이 I블록 위 (5,1)에 스택됨");
        Assert.AreEqual(2, afterSecondDrop.board.GetBlock(6, 1), "O블록이 I블록 위 (6,1)에 스택됨");
        Assert.AreEqual(2, afterSecondDrop.board.GetBlock(5, 2), "O블록이 I블록 위 (5,2)에 스택됨");
        Assert.AreEqual(2, afterSecondDrop.board.GetBlock(6, 2), "O블록이 I블록 위 (6,2)에 스택됨");

        // 세 번째 블록: T블록을 다른 위치에 Drop
        var thirdBlock = new Tetrimino(TetriminoType.T);
        ...
        Assert.AreEqual(1, afterThirdDrop.board.GetBlock(4, 0), "I블록이 여전히 제자리에 있음");
        Assert.AreEqual(2, afterThirdDrop.board.GetBlock(5, 1), "O블록이 여전히 제자리에 있음");

        // T블록이 왼쪽 바닥에 고정되었는지 확인
        Assert.AreEqual(3, afterThirdDrop.board.GetBlock(0, 0), "T블록 좌측 부분 (0,0)에 고정됨");
        Assert.AreEqual(3, afterThirdDrop.board.GetBlock(1, 0), "T블록 중앙 부분 (1,0)에 고정됨");
        Assert.AreEqual(3, afterThirdDrop.board.GetBlock(2, 0), "T블록 우측 부분 (2,0)에 고정됨");
        Assert.AreEqual(3, afterThirdDrop.board.GetBlock(1, 1), "T블록 상단 부분 (1,1)에 고정됨");

        // 네 번째 블록: 스택된 영역 옆에 또 다른 블록 Drop
        var fourthBlock = new Tetrimino(TetriminoType.I);
        ...
        Assert.AreEqual(1, afterFourthDrop.board.GetBlock(1, 2), "세로 I블록이 T블록 위 (1,2)에 스택됨");
        Assert.AreEqual(1, afterFourthDrop.board.GetBlock(1, 3), "세로 I블록이 T블록 위 (1,3)에 스택됨");
        Assert.AreEqual(1, afterFourthDrop.board.GetBlock(1, 4), "세로 I블록이 T블록 위 (1,4)에 스택됨");
        Assert.AreEqual(1, afterFourthDrop.board.GetBlock(1, 5), "세로 I블록이 T블록 위 (1,5)에 스택됨");

        // 기존 블록들이 영향받지 않았는지 확인
        Assert.AreEqual(3, afterFourthDrop.board.GetBlock(1, 1), "T블록이 여전히 제자리에 있음");
        Assert.AreEqual(2, afterFourthDrop.board.GetBlock(5, 1), "O블록이 여전히 제자리에 있음");
    }
    ...
    public void DropTetrimino_ComplexStacking_ShouldHandleDifferentHeights()
    {
        ...
        // 1단계: 왼쪽에 높은 타워 만들기 (세로 I블록 두 개)
        var tower1 = new Tetrimino(TetriminoType.I);
        ...
        var tower2 = new Tetrimino(TetriminoType.I);
        ...
        var midBlock = new Tetrimino(TetriminoType.O);
        ...
        // 3단계: 오른쪽에 낮은 블록 (T블록)
        var lowBlock = new Tetrimino(TetriminoType.T);
        ...
        var wideBlock = new Tetrimino(TetriminoType.I);
        ...
        {
            // 가로 I블록의 실제 착지 위치 확인 (x=1,2,3,4)
            if (finalData.board.GetBlock(1, y) == 1 &&
                finalData.board.GetBlock(2, y) == 1 &&
                finalData.board.GetBlock(3, y) == 1 &&
                finalData.board.GetBlock(4, y) == 1)
            {
                foundWideBlock = true;
    ...
    public void MoveTetrimino_MultipleRotations_ShouldWorkCorrectly()
    {
        ...
        Assert.AreEqual(7, updatedGameData.board.GetBlock(5, 0), "270도 회전된 L블록 (5,0)에 고정됨");
        Assert.AreEqual(7, updatedGameData.board.GetBlock(5, 1), "270도 회전된 L블록 (5,1)에 고정됨");
        Assert.AreEqual(7, updatedGameData.board.GetBlock(5, 2), "270도 회전된 L블록 (5,2)에 고정됨");        // 4번째 블록 위치 확인 - 실제로는 (4,2)에 있음
        Assert.AreEqual(7, updatedGameData.board.GetBlock(4, 2), "270도 회전된 L블록 4번째 블록 (4,2)에 고정됨");

        bool foundLBlock = false;
        for (int y = 0; y < 5; y++) // 하위 몇 줄에서 L블록 찾기
        {
            if (updatedGameData.board.GetBlock(5, y) == 7) // L블록 = 7 (enum 순서: I,O,T,S,Z,J,L = 0~6, +1 = 1~7)
            {
                foundLBlock = true;
                break;
            }
